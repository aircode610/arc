...
# filename = "E:\\amirparsa\\web\\html css\\5\\equations.txt"
...
for k in range(21) :
    r2 = k*dr+r1
    for t1 in range(0, 178):
        for t2 in range(t1+1, 179):
            for t3 in range(t2+1, 180):
                # for line in lines:
                #     try:
                #         exec(line)
                #     except:
                ...
                try: 
                    ...
                    sn = an*(r2**2-r1**2)
                    nc = sqrt(rn**2+r2**2-(2*rn*r2*cos(radians(an))))
                    nb = sqrt(rn**2+r1**2-(2*rn*r1*cos(radians(an))))
                    cos_NCB = (nc**2+BC**2-nb**2) / (2*nc*BC)
                    sin_NCB = sqrt(1-cos_NCB**2)
                    sin_OCB = r1/BC*sin(radians(t3-t2))
                    cos_OCB = sqrt(1-sin_OCB**2)
                    cos_NCI = -cos_OCD*cos_OCB*cos_NCB+sin_OCD*sin_OCB*cos_NCB+sin_OCD*cos_OCB*sin_NCB+cos_OCD*sin_OCB*sin_NCB #cos_OCB?
                    ...
                    Wm = -sm*Am*cos_MAO
                    Wn = -sn*(In/IB)*AB*cos(radians((t2+t3)/2))
                    Wp = sp*AB*(IC/IB)*(dp/CD)*cos_POX
                    Wt = Wm+Wn+Wp
                    WY_me = sm*Am*sin_MAO
                    WY_ne = sn*(In/IB)*AB*sin(radians((t2+t3)/2))
