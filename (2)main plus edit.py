from math import sqrt, cos, sin, radians, asin, acos, pi
import xlsxwriter
    ...
    for t1 in range(0, 46):
        for t2 in range(60, 81):
            for t3 in range(120, 161):
                ...
                v = (2)*((asin(sin_OCD)+asin(sin_OCB)+asin(sin_NCB)))*(Pi)/pi
                sin_BAO= (r1*sin_ABO)/r2
                IA= AB + IB
                IO= sqrt(IA**2+r2**2-2*IA*r2*abs(cos(asin(sin_BAO))))
                if IO>r2:
                    NCI =Pi-((asin(sin_OCD)+asin(sin_BCO)+asin(sin_NCB))*Pi/pi)
                else :
                    NCI =(asin(sin_OCD)+asin(sin_BCO)+asin(sin_NCB))*Pi/pi
                ...
                f = (asin(sin_INC))*Pi/pi
                sin_CNO =(r2 / nc) * sin(radians((t3-t2)/2))
                g = abs((asin(sin_CNO))*Pi/pi)
                if IO>r2:
